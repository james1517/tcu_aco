...
for (let y = 0; y < AXIS; y++) {
    ...
    for (let z = 0; z < NODE_NUMBER; z++) {
        if (z == 0) {
            ...
            route_y[y][z] = Math.floor(Math.random() * (RANDOM_MAX_Y + 1 - RANDOM_MIN_Y)) + route_y[y][z - 1] + 50;
        }
...
function moveAnt(ant_num, node_num) {
    ede[ant_num].x += (ede[node_num].x - ede[ant_num].last_pos_x);
    ede[ant_num].y += (ede[node_num].y - ede[ant_num].last_pos_y);
    //ant[ant_num].x += (ede[node_num].x - ede[ant_num].last_pos_x) / ede[node_num].length;
    //ant[ant_num].y += (ede[node_num].y - ede[ant_num].last_pos_y) / ede[node_num].length;
    //console.log("x: "+ede[ant_num].x);
    //console.log("y: "+ede[ant_num].y);
}
...
function drawAnt() {
    ...
    act.beginPath();
    act.arc(ede[0].x, ede[0].y, 10, 0, 2 * Math.PI);
    act.stroke();
    act.closePath();
